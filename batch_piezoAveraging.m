...
for kk = 1:length(directories)
    ...
    saveTag = dir([dataDir '*removeY7.txt']);
    if length(saveTag) > 1
        warning('Using first found *removeY7.txt file')
    elseif isempty(saveTag)
        warning(['No *removeY7.txt file found. ' 10 dataDir ' will be skipped'])
        continue
    end
    saveTag = saveTag(1).name(1:end-4);

     % Run with z-filtering
    filtZ = 1;
    if ~exist([dataDir saveTag '_filtZ' num2str(zThresh) '_piezoAveragingWorkspace.mat'],'file') || overwrite
        piezoAveraging(dataDir,saveTag,filtZ,zThresh,0)
        disp(' ')
        disp(' ')
    else
